...
//				 || ( value < 0 ) || ( ff == null ))
    ...
    public double calculateUtility(double alpha, double beta) {
        double fv = getFitness(); // Điều chỉnh tùy thuộc vào cách tính giá trị thích nghi
        double cbv = calculateBranchCoverage(); // Điều chỉnh tùy thuộc vào cách tính giá trị bao phủ nhánh

        return alpha * fv + beta * cbv;
    }

    // Phương thức tính giá trị bao phủ nhánh (cb_v)
    private double calculateBranchCoverage() {
        // Giả sử giá trị bao phủ nhánh được tính bằng nghịch đảo của giá trị trung bình của coverageValues
        double totalCoverage = coverageValues.values().stream().mapToDouble(Double::doubleValue).sum();
        double averageCoverage = totalCoverage / coverageValues.size();

        // Bảo đảm rằng averageCoverage không bằng 0 để tránh chia cho 0
        if (averageCoverage == 0) {
            averageCoverage = 1.0; // Giá trị mặc định nếu không có giá trị bao phủ
        }
